...
__global__ void pagerankPullDualKernel(
    ...
    if (active) {
        ...
        mainVal = (1.0f - PR_ALPHA) + PR_ALPHA * sum;
    }
    __syncthreads();
    ...
    if (idle && iid < critical_count && iid < BLOCK_SIZE) {
        ...
        redundant_results[iid] = (1.0f - PR_ALPHA) + PR_ALPHA * sum;
    }
    __syncthreads();
...
void pagerankGPU(
    ...
    for (int i = 0; i < CHECK_BUFFER_COUNT; ++i) {
        CUDA_CHECK(cudaMallocHost(&h_compact_delta[i], num_nodes * sizeof(float)));
    ...
    float* d_compact_delta[CHECK_BUFFER_COUNT];
    float* d_compact_delta_scratch;
    ...
    for (int i = 0; i < CHECK_BUFFER_COUNT; ++i) {
        CUDA_CHECK(cudaMalloc(&d_compact_delta[i], num_nodes * sizeof(float)));
        CUDA_CHECK(cudaMalloc(&d_info[i], sizeof(MonotonicInfo)));
    }
    CUDA_CHECK(cudaMalloc(&d_compact_delta_scratch, num_nodes * sizeof(float)));
                ...
                if (result.count_update > 0) {
                    {
                        ...
                        CUDA_CHECK(cudaMemcpyAsync(
                            ...
                            result.count_update * sizeof(float), cudaMemcpyDeviceToHost, delta_copy_stream
                        ));
                    ...
                    }
                    {
                        ...
                        for (int i = 0; i < result.count_update; ++i) {
                            result.sum_delta += (double)h_compact_delta[task.buf][i];
                        }
                    }
                    double avg_delta = result.sum_delta / (double)result.count_update;
    ...
    while (iter < 1000) {
        ++iter;
        {
            ...
            float* iter_compact_delta = do_async_check ? d_compact_delta[check_buf] : d_compact_delta_scratch;
    ...
    CUDA_CHECK(cudaFree(d_compact_delta_scratch)); CUDA_CHECK(cudaFree(d_info_scratch)); CUDA_CHECK(cudaFreeHost(h_active));
