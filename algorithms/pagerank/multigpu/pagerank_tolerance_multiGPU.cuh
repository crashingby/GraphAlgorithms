...
#define PR_ALPHA_MGPU 0.85f
#define PR_TOL_MGPU 1e-3f

__global__ void pagerankPartitionKernel(float* values, const int* local_outdegree, const int* ro, const int* ci, const int* co, const int* ri, const int* active, int* update, int owned_count, int local_count, float* delta, MgCheckInfo* check_info) {
int tid=blockIdx.x*blockDim.x+threadIdx.x;
int local_tid=threadIdx.x;
__shared__ int critical_list[BLOCK_SIZE];
__shared__ int idle_count, critical_count;
...
__syncthreads();
float old=valid?values[tid]: 0.0f;
float nv=old;
if (is_active) {
float sum=0.0f;
for (int i=co[tid];
...
int out=local_outdegree[src];
if (out>0) sum+=values[src]/(float)out;
}
} nv=(1.0f-PR_ALPHA_MGPU)+PR_ALPHA_MGPU*sum;
}
__syncthreads();
...
} redundant_results[iid]=(1.0f-PR_ALPHA_MGPU)+PR_ALPHA_MGPU*sum;
}
__syncthreads();
if (critical && cid>=0 && cid<idle_count && cid<BLOCK_SIZE && fabsf(redundant_results[cid]-nv)>1e-5f) atomicExch(&check_info->dmr_error_flag, 1);
__syncthreads();
if (is_active) {
values[tid]=nv;
float diff=fabsf(nv-old);
delta[tid]=diff;
if (diff>=PR_TOL_MGPU) {
for (int i=ro[tid];
i<ro[tid+1];
...
} else if (valid) delta[tid]=0.0f;
}

inline void pagerankMultiGPU(float* h_value, const int* h_ro, const int* h_ci, const int* h_co, const int* h_ri, int num_nodes, int num_edges, float alpha=0.5f, float beta=0.5f, float threshold=0.3f) {
(void)num_edges;
(void)alpha;
(void)beta;
int ng=0;
...
} printf("发现 %d 个 GPU，开启 NCCL 子图划分 PageRank：owned + ghost。\n", ng);
...
std::vector<float*> values(ng), check_delta(ng), scratch_delta(ng);
std::vector<MgCheckInfo*> check_info(ng), scratch_info(ng);
std::vector<float*> h_check_delta(ng*2, nullptr);
...
std::vector<int*> local_outdegree(ng), active(ng), next_active(ng), update(ng), ro(ng), ci(ng), co(ng), ri(ng), num_active(ng);
...
CUDA_CHECK(cudaMalloc(&values[d], la*sizeof(float)));
CUDA_CHECK(cudaMalloc(&local_outdegree[d], la*sizeof(int)));
...
CUDA_CHECK(cudaMalloc(&check_delta[d], oa*sizeof(float)));
CUDA_CHECK(cudaMalloc(&scratch_delta[d], oa*sizeof(float)));
...
CUDA_CHECK(cudaHostAlloc(&h_check_delta[slot], oa*sizeof(float), cudaHostAllocDefault));
...
std::vector<float> hv(la, 1.0f/(float)num_nodes);
std::vector<int> hdeg(la, 0), ha(oa, 1);
for (int l=0;
l<p.local_node_count;
++l) {
int g=p.local_to_global[l];
hdeg[l]=h_ro[g+1]-h_ro[g];
}
CUDA_CHECK(cudaMemcpy(values[d], hv.data(), la*sizeof(float), cudaMemcpyHostToDevice));
CUDA_CHECK(cudaMemcpy(local_outdegree[d], hdeg.data(), la*sizeof(int), cudaMemcpyHostToDevice));
...
mg_alloc_float(&q.d_value_send_buf_float, q.value_send_count);
mg_alloc_float(&q.d_value_recv_buf_float, q.value_recv_count);
}
} auto pack_act=[&](int d) {
CUDA_CHECK(cudaSetDevice(d));
for (int peer=0;
...
mgPackFloatKernel<<<b, BLOCK_SIZE, 0, streams[d]>>>(values[d], q.d_value_send_owned, q.d_value_send_buf_float, q.value_send_count);
}
}
};
...
mgUnpackFloatKernel<<<b, BLOCK_SIZE, 0, streams[d]>>>(q.d_value_recv_buf_float, q.d_value_recv_ghost, values[d], q.value_recv_count);
}
}
};
auto xact=[&]() {
...
if (peer==d)continue;
auto& q=dp[d][peer];
if (q.value_recv_count>0) NCCL_CHECK(ncclRecv(q.d_value_recv_buf_float, q.value_recv_count, ncclFloat, peer, comms[d], streams[d]));
if (q.value_send_count>0) NCCL_CHECK(ncclSend(q.d_value_send_buf_float, q.value_send_count, ncclFloat, peer, comms[d], streams[d]));
}
}
NCCL_CHECK(ncclGroupEnd());
};
...
mgScoreAndMarkFloatKernel<<<b, BLOCK_SIZE, 0, streams[d]>>>(active[d], values[d], ro[d], parts[d].owned_count, max_outdegree, alpha, beta, threshold, num_active[d]);
}
}
for (int d=0;
d<ng;
...
for (int i=0;
i<p.owned_count;
++i) r.sum_delta+=(double)h_check_delta[slot][i];
...
float* iter_delta=do_check?check_delta[d]: scratch_delta[d];
...
pagerankPartitionKernel<<<b, BLOCK_SIZE, 0, streams[d]>>>(values[d], local_outdegree[d], ro[d], ci[d], co[d], ri[d], active[d], update[d], p.owned_count, p.local_node_count, iter_delta, iter_info);
...
CUDA_CHECK(cudaMemcpyAsync(h_check_delta[slot], iter_delta, p.owned_count*sizeof(float), cudaMemcpyDeviceToHost, check_streams[d]));
...
if (parts[d].owned_count>0) {
int b=(parts[d].owned_count+BLOCK_SIZE-1)/BLOCK_SIZE;
mgScoreAndMarkFloatKernel<<<b, BLOCK_SIZE, 0, streams[d]>>>(next_active[d], values[d], ro[d], parts[d].owned_count, max_outdegree, alpha, beta, threshold, num_active[d]);
}
...
int dmr_flag=0, residual_increase_flag=0, first_dmr=-1, first_residual=-1;
...
if (prev_avg<1e99 && avg>prev_avg*(1.0+(double)threshold) && first_residual<0) {
residual_increase_flag=1;
first_residual=it;
} prev_avg=avg;
}
} printf("异步CPU检测(PageRank): DMR=%d(first_iter=%d), residual_increase=%d(first_iter=%d), monotonic=N/A\n", dmr_flag, first_dmr, residual_increase_flag, first_residual);
for (int d=0;
d<ng;
++d) {
CUDA_CHECK(cudaSetDevice(d));
...
std::vector<float> out(p.owned_count);
CUDA_CHECK(cudaMemcpy(out.data(), values[d], p.owned_count*sizeof(float), cudaMemcpyDeviceToHost));
for (int l=0;
l<p.owned_count;
...
printf("NCCL 子图划分 PageRank 迭代 %d 次结束。\n", iter);
for (int d=0;
d<ng;
++d) {
CUDA_CHECK(cudaSetDevice(d));
cudaFree(values[d]);
cudaFree(local_outdegree[d]);
cudaFree(active[d]);
...
cudaFree(q.d_value_send_buf_float);
cudaFree(q.d_value_recv_buf_float);
