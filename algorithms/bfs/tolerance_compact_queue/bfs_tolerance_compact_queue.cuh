...
struct MonotonicInfo {
    ...
    int count_update;         // 本轮紧凑 delta 数组中的有效元素个数
};
...
__global__ void bfsPullDualKernel(
    ...
    int num_nodes,
    int* d_compact_delta, // 只保存本轮发生更新的 delta
...
void bfsGPU(
    ...
    printf("开始 GPU BFS（紧凑delta队列检测容错实现）...\n"); fflush(stdout);
    ...
    for (int i = 0; i < CHECK_BUFFER_COUNT; ++i) {
        cudaMallocHost(&h_compact_delta[i], num_nodes * sizeof(int));
    ...
    for(int i = 0; i < CHECK_BUFFER_COUNT; i++){
        cudaMalloc(&d_compact_delta[i], num_nodes * sizeof(int));
        cudaMalloc(&d_info[i], sizeof(MonotonicInfo));
    }
    cudaMalloc(&d_compact_delta_scratch, num_nodes * sizeof(int));
    ...
    cudaStreamCreateWithFlags(&delta_copy_stream, cudaStreamNonBlocking);
            ...
            if (pending_tasks.peek(task)) {
                ...
                if (cudaEventQuery(check_done_event[buf]) == cudaSuccess) {
                    ...
                    if (result.count_update > 0) {
                        {
                            NvtxRange copy_range(nvtx_name("worker D2H compact delta", iter_id));
                            cudaMemcpyAsync(
                                h_compact_delta[buf],
                                d_compact_delta[buf],
                                result.count_update * sizeof(int),
                                cudaMemcpyDeviceToHost,
                                delta_copy_stream
                            );
                            cudaStreamSynchronize(delta_copy_stream);
                        }

                        {
                            NvtxRange scan_compact_range(nvtx_name("worker CPU scan compact delta", iter_id));
                            for (int i = 0; i < result.count_update; ++i) {
                                result.sum_delta += (long long)h_compact_delta[buf][i];
                            }
                        }
    ...
    while(iter < 1000) {
        iter++;
        {
            ...
            int* iter_compact_delta = do_async_check
                ? d_compact_delta[check_buf]
                : d_compact_delta_scratch;
            ...
            bfsPullDualKernel<<<num_blocks, BLOCK_SIZE, 0, compute_stream>>>(
                ...
                iter_compact_delta, iter_info, TREND_DEC
            );
            ...
            if (do_async_check) {
                ...
                if (!pending_tasks.try_push({iter, check_buf})) {
                    ...
                    cudaEventSynchronize(check_done_event[check_buf]);
    ...
    for (int i = 0; i < CHECK_BUFFER_COUNT; i++) {
        cudaFree(d_compact_delta[i]);
        cudaFree(d_info[i]);
    }
    cudaFree(d_compact_delta_scratch);
    cudaFree(d_info_scratch);
    cudaFreeHost(h_active);
    for (int i = 0; i < CHECK_BUFFER_COUNT; i++) {
        cudaFreeHost(h_compact_delta[i]);
    ...
    cudaStreamDestroy(delta_copy_stream);
