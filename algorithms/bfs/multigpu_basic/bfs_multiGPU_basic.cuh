...
inline void bfsMultiGPUBasic(
    ...
    if (rank == 0) {
        printf("发现 %d 个 MPI rank，开启 BFS 多节点无容错版本。\n", world_size);
    }
    printf(
    ...
    std::vector<int> h_local_value(part.local_node_count, BFS_MN_INF);
    for (int lv = 0; lv < part.local_node_count; ++lv) {
        if (part.local_to_global[lv] == src) h_local_value[lv] = 0;
    }

    // Pull BFS must initially schedule vertices that can observe the source.
    // Both checked and unchecked implementations use this identical work set.
    ...
    // Exclude initialization and the first ghost-value fill from all loop metrics.
    ...
    while (iter < 1000 && total_active > 0) {
        ...
        bfsDistributedPullKernel<<<dg_blocks(part.owned_count), DG_BLOCK_SIZE, 0, stream>>>(
            d_values,
            d_row_offsets,
            d_column_indices,
            d_column_offsets,
