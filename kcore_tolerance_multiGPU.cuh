...
__global__ void kcorePartitionKernel(int* values, int* alive, const int* ro, const int* ci, const int* co, const int* ri, const int* active, int* update, int owned_count, int local_count, int k, int* delta, MgCheckInfo* check_info) {
...
++i) {
int src=ri[i];
if (src>=0 && src<local_count && alive[src]!=0) deg++;
}
}
__syncthreads();
...
++i) {
int src=ri[i];
if (src>=0 && src<local_count && alive[src]!=0) r++;
} redundant_results[iid]=r;
}
__syncthreads();
if (critical && cid>=0 && cid<idle_count && cid<BLOCK_SIZE && redundant_results[cid]!=deg) atomicExch(&check_info->dmr_error_flag, 1);
__syncthreads();
if (is_active) {
values[tid]=deg;
int diff=deg-old;
if (deg<k && alive[tid]!=0) {
alive[tid]=0;
for (int i=ro[tid];
i<ro[tid+1];
++i) {
int dst=ci[i];
if (dst>=0 && dst<local_count && alive[dst]!=0) update[dst]=1;
} delta[tid]=diff;
...
inline void kcoreMultiGPU(int* h_value, const int* h_ro, const int* h_ci, const int* h_co, const int* h_ri, int num_nodes, int num_edges, int k, float alpha=0.5f, float beta=0.5f, float threshold=0.3f) {
(void)num_edges;
(void)alpha;
(void)beta;
int ng=0;
...
} printf("发现 %d 个 GPU，开启 NCCL 子图划分 KCORE：owned + ghost。\n", ng);
...
std::vector<int*> values(ng), alive(ng), active(ng), next_active(ng), update(ng), ro(ng), ci(ng), co(ng), ri(ng), num_active(ng), check_delta(ng), scratch_delta(ng);
...
CUDA_CHECK(cudaMalloc(&alive[d], la*sizeof(int)));
...
std::vector<int> hv(la), halive(la, 1), ha(oa, 1);
for (int l=0;
l<p.local_node_count;
++l) {
int g=p.local_to_global[l];
hv[l]=h_ro[g+1]-h_ro[g];
}
CUDA_CHECK(cudaMemcpy(values[d], hv.data(), la*sizeof(int), cudaMemcpyHostToDevice));
CUDA_CHECK(cudaMemcpy(alive[d], halive.data(), la*sizeof(int), cudaMemcpyHostToDevice));
...
mgPackIntKernel<<<b, BLOCK_SIZE, 0, streams[d]>>>(alive[d], q.d_value_send_owned, q.d_value_send_buf_int, q.value_send_count);
}
}
};
...
mgUnpackIntKernel<<<b, BLOCK_SIZE, 0, streams[d]>>>(q.d_value_recv_buf_int, q.d_value_recv_ghost, alive[d], q.value_recv_count);
}
}
};
auto xact=[&]() {
...
kcorePartitionKernel<<<b, BLOCK_SIZE, 0, streams[d]>>>(values[d], alive[d], ro[d], ci[d], co[d], ri[d], active[d], update[d], p.owned_count, p.local_node_count, k, iter_delta, iter_info);
...
} printf("异步CPU检测(KCORE): DMR=%d(first_iter=%d), monotonic=%d(first_iter=%d), avg_delta_increase=%d(first_iter=%d)\n", dmr_flag, first_dmr, monotonic_flag, first_mono, avg_delta_increase_flag, first_avg);
for (int d=0;
d<ng;
++d) {
CUDA_CHECK(cudaSetDevice(d));
...
printf("NCCL 子图划分 KCORE 迭代 %d 次结束。\n", iter);
for (int d=0;
d<ng;
++d) {
CUDA_CHECK(cudaSetDevice(d));
